...
local function _calculateProgress(self)
  ...
  if reversed then fillSize = math.round(dependingSize - (rangeFraction * dependingSize)) end
  return fillSize
end
...
function ProgressShape:getHeight() return self.size.y end
...
function ProgressShape:drawProgress()
  ...
  Utils.match(self.shape, {
    [ShapeType.RECTANGLE] = function()
      love.graphics.rectangle("fill",0,0,vertical and self:getWidth() or fillSize,
    ...
    [ShapeType.CIRCLE] = function()
      ...
      love.graphics.circle("fill",0,0,vertical and self:getWidth() or fillSize,
        not vertical and self:getHeight() or fillSize)
    end
  })
