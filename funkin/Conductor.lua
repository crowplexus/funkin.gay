...
function Conductor:construct(startingBPM)
  ...
  rawset(self, "crotchet", 0) --- @type number
  rawset(self, "semiquaver", 0) --- @type number
...
function Conductor:set_bpm(vl)
  ...
  rawset(self, "semiquaver", Conductor.calculateCrotchet(vl) * self.stepsPerBeat)
  return rawset(self, "bpm", vl)
end

function Conductor:set_stepsPerBeat(vl)
  rawset(self, "semiquaver", Conductor.calculateCrotchet(self.bpm) * vl)
